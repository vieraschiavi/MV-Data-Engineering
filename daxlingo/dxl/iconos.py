...
Los consume:
  · `app/app.py`            barra de pestañas (máscara CSS)
  · `web/assets/iconos.css` tarjetas de la landing y de descarga
                            (lo genera `web/generar-iconos-css.py`)
"""
from __future__ import annotations

# Las pestañas, en el orden de `st.tabs(...)` de app.py. El nombre es la
# clave: si cambia el orden de las pestañas, acá no hay que tocar nada.
# El ORDEN es el de `st.tabs(...)` en `app/app.py`, y la posición es lo
# único que une un ícono con su pestaña: el CSS los reparte por
# `nth-child`. Faltaba «powerquery», así que desde la octava pestaña en
# adelante cada una mostraba el ícono de la siguiente y las dos últimas
# se quedaban sin ninguno. Nadie lo reportó porque un ícono equivocado no
# rompe nada — simplemente miente. Hay un test que ata esta tupla a las
# pestañas reales para que no se vuelva a desfasar al agregar una.
PESTANAS = (
    "guia", "modelo", "relaciones", "analizador", "generar", "explicador",
    "transformar", "powerquery", "exportar", "frescura", "bitacora", "fabric", "overlay",
    "academia", "herramientas", "licencia", "config",
)
...
CUERPOS: dict[str, str] = {
    ...
    # Un embudo: entra todo, sale lo que pasa los pasos.
    "powerquery": "<path d='M3 5h18l-7 8v6l-4 2v-8z'/>",
    # Un reloj: lo único que este panel mide de verdad es el tiempo.
    "frescura": ("<circle cx='12' cy='12' r='9'/><path d='M12 7v5l3.5 2'/>"),
    # Una hoja con renglones y su esquina doblada: el registro de lo hecho.
    "bitacora": ("<path d='M6 3h9l4 4v14H6z'/><path d='M15 3v4h4'/>"
    ...
    # Los dos modos de instalación (ver `dxl/entorno.py`). Los usa la sección
    # de despliegue de la landing: una laptop para la máquina del analista,
    # un rack para la VM del cliente.
    "escritorio": ("<rect x='3' y='4' width='18' height='12' rx='2'/>"
                   "<path d='M2 20h20'/>"),
    "servidor": ("<rect x='3' y='4' width='18' height='7' rx='2'/>"
