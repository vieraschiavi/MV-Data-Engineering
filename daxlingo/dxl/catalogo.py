...
class Catalogo:
    ...
    def __init__(self) -> None:
        ...
        self.cultura = ""
        # Anotaciones del modelo (p. ej. la empresa propia): lo que el
        # producto decide y necesita recordar viaja adentro del archivo.
        self.anotaciones: dict[str, str] = {}
    ...
    def desde_modelo(cls, modelo: dict) -> "Catalogo":
        ...
        cat.anotaciones = {a.get("name", ""): str(a.get("value", ""))
        ...
        for t in m.get("tables", []):
            ...
            for c in t.get("columns", []):
                columnas.append({
                    ...
                    # Qué columna la ordena. Hace falta para saber si una
                    # columna calculada existe PARA ordenar a otra — el uso
                    # legítimo que R07 no debe castigar.
                    "orden_por": c.get("sortByColumn", ""),
                    # La clave de fecha del calendario: sin ella Power BI
                    # no reconoce la tabla como tabla de fechas.
                    "clave": bool(c.get("isKey")),
                })
            medidas = []
            ...
            cat.tablas.append({
                ...
                # Las jerarquías que el modelo YA declara. Cuando un
                # .pbix las trae, mandan sobre cualquier heurística para
                # armar el desplegable de una matriz: alguien las pensó.
                "jerarquias": [
                    {"nombre": h.get("name", ""),
                     "niveles": [lv.get("column", "")
                                 for lv in sorted(
                                     h.get("levels", []) or [],
                ...
                # Tabla calculada (partición DAX): sus columnas derivadas no
                # tienen un «origen» al que bajarse — R07 no las acusa.
                "calculada": any(
                    (p.get("source") or {}).get("type") == "calculated"
                    for p in t.get("partitions", [])),
                # El Power Query de la tabla: de acá sale de DÓNDE lee, que
                # es lo que R17 mira (una ruta personal que se rompe al
                # mover el archivo o abrirlo en otra PC).
                "origen_m": "\n".join(
                    expr_texto((p.get("source") or {}).get("expression"))
                    for p in t.get("partitions", [])
                    if (p.get("source") or {}).get("type") == "m"),
            })
        ...
        for r in m.get("relationships", []):
            cat.relaciones.append({
                ...
                # Las cardinalidades tienen DEFAULT en TMSL: from=many,
                # to=one. Exigir que "many" viniera escrito de los dos lados
                # hacía invisible la muchos-a-muchos más común — la que
                # Power BI escribe como `toCardinality: many` a secas, con el
                # from implícito. Dos archivos reales la tenían y la regla
                # R10 no la veía.
                "muchos_a_muchos": (
                    r.get("fromCardinality", "many") == "many"
    ...
    def desde_layout(cls, layout: dict) -> "Catalogo":
        ...
        entidades y propiedades que los visuales referencian. No hay tipos ni
        expresiones — y se dice.

        Se recorre el árbol ENTERO de cada visual en vez de leer la ruta fija
        `singleVisual.prototypeQuery.Select`: esa ruta solo existe en el
        formato clásico. En el formato PBIR (el de los archivos nuevos, lo
        que `_leer_pbir` traduce) la consulta vive en `query.queryState` y la
        entidad viene directa en `SourceRef.Entity`, sin alias. Con la ruta
        fija, un .pbix moderno daba un catálogo VACÍO en silencio — «no toma
        nada» — que es exactamente el bug que motivó esta reescritura.
        """
        cat = cls()
        cat.parcial = True
        ...
        def entidad_de(ref: dict, alias: dict) -> str:
            # PBIR: la entidad viene directa. Clásico: viene un alias que la
            # lista `From` más cercana resuelve.
            return ref.get("Entity") or alias.get(ref.get("Source")) or ""

        def anotar(clase: str, ent: str, prop: str) -> None:
            if not (ent and prop):
                return
            t = tabla_de(ent)
            if clase == "Measure":
                if not any(m["nombre"] == prop for m in t["medidas"]):
                    t["medidas"].append({
                        "nombre": prop, "expresion": "", "formato": "",
                        "descripcion": "", "carpeta": "", "tabla": ent})
            elif not any(c["nombre"] == prop for c in t["columnas"]):
                t["columnas"].append({
                    "nombre": prop, "tipo": "", "oculta": False,
                    "formato": "", "calculada": False,
                    "expresion": "", "origen": ""})

        def hurgar(o, alias: dict) -> None:
                ...
                if isinstance(fr, list):
                    ...
                    for f in fr:
                        if isinstance(f, dict) and f.get("Name"):
                            alias[f["Name"]] = f.get("Entity")
                for clave, valor in o.items():
                    if (clave in ("Column", "Measure", "HierarchyLevel")
                            and isinstance(valor, dict)):
                        # Un nivel de jerarquía trae Level, no Property, y la
                        # entidad un nivel más adentro (mismas formas que en
                        # reporte.campos_de).
                        prop = valor.get("Property") or valor.get("Level") or ""
                        exp = valor.get("Expression") or {}
                        ent = ""
                        if isinstance(exp, dict):
                            ent = entidad_de(exp.get("SourceRef") or {}, alias)
                            if not ent:
                                jer = exp.get("Hierarchy") or {}
                                ent = entidad_de(
                                    ((jer.get("Expression") or {})
                                     .get("SourceRef") or {}), alias)
                        anotar(clave, ent, prop)
                    hurgar(valor, alias)
        ...
        for seccion in layout.get("sections", []):
            for vc in seccion.get("visualContainers", []):
                hurgar(_js(vc.get("config")), {})
                # Los filtros del visual también nombran campos reales.
                hurgar(_js(vc.get("filters")), {})
    ...
    def tabla_fechas(self) -> dict | None:
        ...
        for t in self.tablas:
            ...
            if any(p in _norm(t["nombre"])
                ...
                return t
        # Último recurso: una tabla con SU columna de fecha. Pero un
        # calendario es una dimensión — una tabla de hechos con su columna
        # «Fecha» no cuenta, por más que se llame igual: si está del lado
        # «muchos» de alguna relación o tiene medidas, es otra cosa.
        lados_muchos = {_norm(r["desde_tabla"]) for r in self.relaciones}
        for t in self.tablas:
            if t["interna"] or t["medidas"] \
                    or _norm(t["nombre"]) in lados_muchos:
                continue
