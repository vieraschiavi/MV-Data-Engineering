...
def banco(ruta: str | Path | None = None) -> dict:
    """El archivo del banco entero, o un banco VACÍO si no está.

    No revienta a propósito. Streamlit corre el script de la app de arriba
    abajo en cada render, así que una excepción acá —en la pestaña Academia,
    la anteúltima— deja la aplicación ENTERA en una pantalla de error: no se
    puede cargar un modelo, ni analizarlo, ni exportar. Es exactamente lo que
    pasó en la edición OWNER portable, que salió empaquetada sin
    `datos/ejercicios.json`: el producto no abría por un archivo de práctica.

    Un banco vacío degrada UNA pestaña y deja el resto del producto en pie,
    que es la proporción correcta entre la falla y su consecuencia.
    """
    ruta = Path(ruta) if ruta else RUTA_EJERCICIOS
    try:
        return json.loads(ruta.read_text(encoding="utf-8"))
    except (FileNotFoundError, json.JSONDecodeError, OSError):
        return {"ejercicios": [], "modelo_practica": None}


def cargar_ejercicios(ruta: str | Path | None = None) -> list[dict]:
    return banco(ruta)["ejercicios"]
